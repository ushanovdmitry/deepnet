...
class DataOpts:
    def __init__(self, opts):
        ...
        self.name = opts.pop("name", "")  # required string name = 1;
        self.file_pattern = opts.pop("filePattern", "")  # required string file_pattern = 2;
        self.size = opts.pop("size", 0)  # required int32 size = 3;
        self.dimensions = opts.pop("dimensions", [])  # repeated int32 dimensions = 4;
        self.num_labels = opts.pop("numLabels", 1)  # optional int32 num_labels = 5 [default=1];
        self.key = opts.pop("key", "")  # optional string key = 6;
        self.sparse = opts.pop("sparse", False)  # optional bool sparse = 7 [default=false];
        self.seq = opts.pop("seq", False)  # optional bool seq = 8 [default=false];
        self.stats_file = opts.pop("statsFile", "")  # optional string stats_file = 9;
...
class DatasetOpts:
    def __init__(self, opts):
        ...
        self.data = [DataOpts(_) for _ in opts.pop("data", [])]  # repeated Data data = 2;
        self.gpu_memory = opts.pop("gpuMemory", "2G")  # optional string gpu_memory = 3 [default="2G"];
        self.main_memory = opts.pop("mainMemory", "4G")  # optional string main_memory = 4 [default="4G"];
        self.data_handler = opts.pop("dataHandler", "deepnet")  # optional string data_handler = 5 [default="deepnet"];
        self.prefix = opts.pop("prefix", "")  # optional string prefix = 6 [default=""];
...
def performance2str(stat, prefix=''):
    s = ''
...
def accumulate_performance(acc, perf):
    """
        :type acc: MetricsOpts
        :type perf: MetricsOpts
    """
