...
def GetDataHandles(op, names, hyp_list, verbose=False):
    ...
    """
    assert isinstance(op, util.OperationOpts)

    typesize = 4
    data_proto_file = os.path.join(op.data_proto_prefix, op.data_proto)
    dataset_opts = util.DatasetOpts(util.load_json(data_proto_file))
    handlers = []
    if dataset_opts.data_handler == 'deepnet':
        size_list = []
        for name_list in names:
            size = 0
            for name in name_list:
                try:
                    data_proto = next(d for d in dataset_opts.data if d.name == name)
    ...
    elif dataset_opts.data_handler == 'navdeep':
        ...
        for i, name_list in enumerate(names):
            if name_list == []:
                ...
                handlers.append(navdeep_datahandler.NavdeepDataHandler(
                    op, dataset_opts, name_list, hyp_list))
...
class DataHandler(object):
    ...
    def __init__(self, op, data_name_list, hyperparameter_list, frac=1.0):
        ...
        for name, hyp in zip(data_name_list, hyperparameter_list):
            data_proto = next(d for d in dataset_opts.data if d.name == name)
            file_pattern = os.path.join(dataset_opts.prefix, data_proto.file_pattern)
            filenames.append(sorted(glob.glob(file_pattern)))
            stats_files.append(os.path.join(dataset_opts.prefix, data_proto.stats_file))
            numdims = np.prod(np.array(data_proto.dimensions))
        ...
        max_gpu_capacity = int(frac*GetBytes(dataset_opts.gpu_memory))
        max_cpu_capacity = int(frac*GetBytes(dataset_opts.main_memory))
        ...
        for i, stats_file in enumerate(stats_files):
            if hyperparameter_list[i].normalize and hyperparameter_list[i].activation != "REPLICATED_SOFTMAX":
